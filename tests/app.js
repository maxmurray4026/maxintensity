...
(async () => {
  ...
  await page.goto('http://localhost:8768/index.html'); await page.waitForTimeout(350); await shot('splash');
  console.log('splash padlock:', await page.locator('#splash svg').count(), '| loading copy anywhere:', /pull down|Loading Max/i.test(await page.innerText('body')));
  await page.waitForTimeout(3200);
  console.log('splash gone after load:', (await page.locator('#splash').count()) === 0);
  ...
  const other = await page.locator('[data-session][data-expanded="false"]').first().getAttribute('data-session');
  await page.getByRole('button', { name: 'Expand ' + other }).click(); await page.waitForTimeout(300);
  console.log('tap ' + other + ' → expanded:', await expanded(), '| collapsed:', await collapsed(), '| exercises:', await page.locator('[data-expanded="true"] ul li').count());
  await shot('train-other');
  await page.getByRole('button', { name: 'Collapse ' + other }).click(); await page.waitForTimeout(300);
  ...
  console.log('calendar cells:', JSON.stringify(dots), '| scheduled solid red:', (await page.locator('[data-status="scheduled"]').first().getAttribute('class')).includes('bg-[#FF2B2B]'), '| no dots:', await page.locator('[data-status] span.rounded-full').count() === 0);
  const strip = await page.getByRole('button', { name: /^Week [1-6]$/ }).evaluateAll((els) => els.map((e) => { const r = e.getBoundingClientRect(); const inner = Array.from(e.querySelectorAll('p')).every((p) => { const q = p.getBoundingClientRect(); return q.left >= r.left - 0.5 && q.right <= r.right + 0.5 && p.scrollWidth <= p.clientWidth + 1; }); return inner; }));
  console.log('week strip labels inside boxes:', strip.every(Boolean), strip.length);
  ...
  await page.getByRole('button', { name: 'No gym today' }).click(); await page.waitForTimeout(300);
  console.log('home version:', (await page.locator('[data-expanded="true"]').innerText()).includes('Home ·'), '| first exercise:', (await page.locator('[data-expanded="true"] ul li').first().innerText()).split('\n').filter(Boolean)[1]);
  await page.getByRole('button', { name: 'Undo — full session' }).click(); await page.waitForTimeout(300);
  ...
  // --- round 5: PLAN MY WEEK ---
  await page.getByRole('button', { name: 'Plan', exact: true }).click(); await page.waitForTimeout(400); await shot('week-1-shop');
  const wk = page.locator('[data-plan-week]');
  await wk.getByRole('button', { name: 'Aldi' }).click(); await wk.getByRole('button', { name: 'Continue' }).click();
  await wk.getByRole('button', { name: 'More people' }).click(); await shot('week-2-people'); await wk.getByRole('button', { name: 'Continue' }).click();
  await wk.getByRole('button', { name: 'Mon', exact: true }).click(); await wk.getByRole('button', { name: 'Continue' }).click();
  await page.locator('[data-plan-week] input[type=range]').fill('70'); await shot('week-4-budget'); await wk.getByRole('button', { name: 'Continue' }).click();
  await wk.getByRole('button', { name: 'Protein packed' }).click(); await wk.getByRole('button', { name: 'Fakeaway' }).click(); await wk.getByRole('button', { name: 'Speedy meals' }).click(); await wk.getByRole('button', { name: 'Low calorie' }).click();
  console.log('moods capped at 3:', await page.locator('[data-plan-week] [aria-pressed="true"]').count()); await shot('week-5-mood'); await wk.getByRole('button', { name: 'Continue' }).click();
  await wk.getByRole('button', { name: 'Hob' }).click(); await wk.getByRole('button', { name: 'Air fryer' }).click(); await wk.getByRole('button', { name: 'Continue' }).click();
  await wk.getByRole('button', { name: 'None', exact: true }).click(); await shot('week-7-diet');
  await wk.getByRole('button', { name: 'Build my week' }).click(); await page.waitForTimeout(900); await shot('week-building');
  console.log('building screen:', await page.locator('[data-building]').count(), '| ticks so far:', await page.locator('[data-building] .bg-\\[\\#FF2B2B\\]').count());
  await page.waitForSelector('[data-week-days]', { timeout: 8000 }); await shot('week-plan');
  const wkTxt = await wk.innerText();
  console.log('week plan: days', await page.locator('[data-day]').count(), '| source:', wkTxt.includes('built by the coach') ? 'coach' : 'library', '| breakfast yogurt:', wkTxt.includes('Greek yogurt'), '| stars:', await page.locator('[data-day] svg').count());
  await page.locator('[data-day="Mon"] button').first().click(); await page.waitForTimeout(400); await shot('week-detail');
  const det = await page.locator('[data-meal-detail]').innerText();
  console.log('meal detail:', ['KCAL', 'INGREDIENTS', 'INSTRUCTIONS', 'YOUR NOTES'].map((k) => k + '=' + det.toUpperCase().includes(k)).join(' '), '| swap btn:', await page.getByRole('button', { name: 'Swap this meal' }).count());
  const before1 = (await page.locator('[data-day="Mon"]').innerText()).split('\n')[1];
  await page.getByRole('button', { name: 'Swap this meal' }).click(); await page.waitForTimeout(300);
  await page.getByRole('button', { name: 'Close', exact: true }).first().click({ position: { x: 10, y: 10 } }); await page.waitForTimeout(300);
  console.log('swapped Mon:', before1, '→', (await page.locator('[data-day="Mon"]').innerText()).split('\n')[1]);
  await wk.getByRole('tab', { name: 'Grocery list' }).click(); await page.waitForTimeout(300); await shot('week-grocery');
  console.log('grocery aisles:', (await page.locator('[data-grocery] .dp').allInnerTexts()).join(' · '));
  await wk.getByRole('button', { name: 'Numbers, timing & foods' }).click(); await page.waitForTimeout(400); await shot('plan-sheet');
