...
(async () => {
  ...
  await page.waitForFunction(async () => (await caches.keys()).includes('mi-shell-v5') && !!(await caches.match('/index.html')) && !!(await caches.match('/offline.html')), null, { timeout: 15000 });
  await page.waitForTimeout(800);
  const cached = await page.evaluate(() => caches.open('mi-shell-v5').then((c) => c.keys()).then((ks) => ks.map((r) => new URL(r.url).pathname)));
